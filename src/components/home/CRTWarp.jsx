...
void main() {
  ...
  vec3 baseWaveColor = mix(uColor, uSecondaryColor, clamp(curvedUv.x, 0.0, 1.0));
  vec3 waveColor = baseWaveColor * (0.3 + signal * 0.7 + glow * uBloom * 0.65);
  ...
  vec3 bgGradient = mix(uBackgroundColor, baseWaveColor, 0.04);
  vec3 color = mix(bgGradient, waveColor, waveMask);
...
export default function CRTWarp({
  ...
  const resolvedBg = backgroundColor === 'auto' ? (isDark ? '#0b0d11' : '#f5f5f1') : backgroundColor;
  const effectiveSecondary = secondaryColor || color;
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uSecondaryColor: { value: new THREE.Color(effectiveSecondary) },
        uBackgroundColor: { value: new THREE.Color(resolvedBg) },
  ...
  useEffect(() => {
    ...
    uniforms.uSecondaryColor.value.set(effectiveSecondary);
    uniforms.uBackgroundColor.value.set(resolvedBg);
